...
import { catchError, map, merge, startWith, switchMap, tap, of as observableOf, Observable, observable } from 'rxjs';
import { BaseComponent } from 'src/app/base/base.component';
import { IAgencySeparationChart } from 'src/app/_models/IAgencySeparationChart';
import { IAgencySeparationSummary, ISeparationTable } from 'src/app/_models/IAgencySeparationSummary';
...
export class SeparationComponent extends BaseComponent<ISeparationTable> implements AfterViewInit, OnInit {
  ...
  ngAfterViewInit(): void {
    ...
    merge(this.filterAction$)
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.staffService.agencySeparation$(this.agencySeparationParam);
        })).subscribe({
          next: (_data) => {
            this.getData$(_data).subscribe({
    ...
    merge(this.filterAction$)
      .pipe(
        ...
        switchMap(() => {
          //this.separationChart$.subscribe();
          return this.chartService.agencySeparationChart$(this.agencySeparationParam);
        })).subscribe({
          next: (_data) => {
            ...
            Array.isArray(_data) ? _data.forEach((chart: IAgencySeparationChart) => {
  ...
  private getData$(_data: IAgencySeparationSummary | null): Observable<ISeparationTable[]> {
    return new
      Observable((observer) => {
        ...
        if (Array.isArray(_data)) {
          ...
          //prepare -reasons
          _data.forEach((row) => {
            if (this.displayedColumns.indexOf(row.monthName.substr(0, 3).toLowerCase()) === -1) {
              this.displayedColumns.push(row.monthName.substr(0, 3).toLowerCase());
            }

            this.titles = row.title;

            if ((_separationTable.length === 0) || (_separationTable.length > 0 && !this.find(_separationTable, row.reasonDesc))) {
              _separationTable.push({ reason: row.reasonDesc, jan: 0, feb: 0, mar: 0, apr: 0, may: 0, jun: 0, jul: 0, aug: 0, sep: 0, oct: 0, nov: 0, dec: 0, grandTotal: 0 });
            }
          });
          ...
          _data.forEach((row) => {
            _separationTable.filter((obj) => {
              if (obj.reason == row.reasonDesc) {
                (row.month === 1) ? obj.jan += row.count : (row.month === 2) ? obj.feb += row.count : (row.month === 3) ? obj.mar += row.count : (row.month === 4) ? obj.apr += row.count : (row.month === 5) ? obj.may += row.count : (row.month === 6) ? obj.jun += row.count : (row.month === 7) ? obj.jul += row.count : (row.month === 8) ? obj.aug += row.count : (row.month === 9) ? obj.sep += row.count : (row.month === 10) ? obj.oct += row.count : (row.month === 11) ? obj.nov += row.count : (row.month === 12) ? obj.dec += row.count : "";
              }
            });
          });
          ...
          _separationTable.forEach(row => {
            ...
            row.grandTotal = (row.jan ?? 0) + (row.feb ?? 0) + (row.mar ?? 0) + (row.apr ?? 0) + (row.may ?? 0) + (row.jun ?? 0) + (row.jul ?? 0) + (row.aug ?? 0) + (row.sep ?? 0) + (row.oct ?? 0) + (row.nov ?? 0) + (row.dec ?? 0);
          });

          this.resultsLength = _separationTable.length;

          console.log('this.separationTable', _separationTable);
  ...
  onSearch() {
    ...
    this.agencySeparationParam.isCalenderYear = this.selectedCalenderType === 'Calender';
